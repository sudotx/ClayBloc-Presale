...
contract MiyaTeesAuction {
    ...
    constructor(address _miyaTees, uint96 reservePrice, uint8 reservePercentage) {
    ...
    function bidTees(uint256 id) external payable {
        require(gasleft() > 150000, "Gas not enough!");

        // auction creation and settlement
        if (hasEnded()) {
            // if auction has ended, and is settled. try creating a new one
            if (!_auctionData.settled && _auctionData.bidder != address(0)) {
        ...
        require(msg.value >= amount + _auctionData.bidIncrement, "Bid below current bid increment!");
